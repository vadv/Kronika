...
fn read_validated_zms_summary<R: ReadAt>(
    reader: &R,
    retained_metadata: usize,
...
const fn invalid_zms(reason: InvalidZmsReason) -> Result<CatalogSummary, ZmsInvalid> {
    Err(ZmsInvalid {
        reason,
        failure: None,
    })
...
fn verify_zms_magic<R: ReadAt>(reader: &R) -> Result<(), StoreError> {
...
mod tests {
    ...
    fn write_journal(root: &Path, raw_id: i64, parts: &[Vec<u8>]) {
        fs::write(root.join("active.wal"), journal(raw_id, parts))
