...
pub const QUARANTINE_DIRECTORY_NAME: &str = ".kronika-quarantine-v1";
...
const ROOT_GENERATION_PREFIX: &str = ".kronika-generation-v1.";
const ROOT_GENERATION_SUFFIX: &str = ".wal";
const ROOT_EVIDENCE_PREFIX: &str = ".kronika-evidence-v1.";
...
#[cfg(test)]
std::thread_local! {
    static ZMS_PUBLISH_FAULT: Cell<Option<(ZmsPublishFaultPoint, i32)>> =
        const { Cell::new(None) };
...
#[cfg(test)]
impl Drop for ZmsPublishFaultGuard {
    fn drop(&mut self) {
        ZMS_PUBLISH_FAULT.with(|fault| fault.set(None));
    }
}
...
#[cfg(test)]
fn arm_zms_publish_fault(point: ZmsPublishFaultPoint, raw_os_error: i32) -> ZmsPublishFaultGuard {
    ZMS_PUBLISH_FAULT.with(|fault| {
        assert!(fault.replace(Some((point, raw_os_error))).is_none());
    });
    ZmsPublishFaultGuard
}
...
#[cfg(test)]
fn inject_zms_publish_fault(point: ZmsPublishFaultPoint) -> io::Result<()> {
    ZMS_PUBLISH_FAULT.with(|fault| match fault.get() {
...
macro_rules! zms_publish_failpoint {
    ($point:ident) => {
        #[cfg(test)]
        inject_zms_publish_fault(ZmsPublishFaultPoint::$point)?;
    };
}
...
pub enum QuarantineReason {
    ...
    ForeignEntry = 1,
    /// A ZMS failed startup catalog or format validation.
    InvalidZms = 2,
...
pub enum QuarantineFailureStage {
    ...
    RootDirectorySync,
    /// A fallback could not promote the fresh generation to `active.wal`.
    JournalPromotion,
...
/// Open, stable descriptor for one `Kronika` data root.
#[derive(Debug, Clone)]
pub struct DataRoot {
    directory: Arc<File>,
...
pub enum EvidenceLocation {
    /// The damaged object still occupies canonical `active.wal`.
    Canonical,
...
pub enum JournalActivation {
    /// Atomic `RENAME_EXCHANGE` installed it at `active.wal`.
    Exchanged,
...
impl JournalRotation {
    ...
    fn move_canonical_to_pending_name(&mut self) -> bool {
        let Some(destination) =
            available_evidence_name(&self.root.directory, self.nonce, c"active.wal")
        else {
            self.diagnostics.push(QuarantineFailure::local(
                QuarantineFailureStage::CollisionSlotsExhausted,
                io::ErrorKind::AlreadyExists,
...
impl WriterOwner {
    ...
    /// wrong-type `active.wal`. The caller initializes the returned file
    ...
    pub fn quarantine_invalid_zms(&self, segment: SegmentArtifacts) -> QuarantineOutcome {
        ...
        self.quarantine_entry(&path, source, QuarantineReason::InvalidZms)
    }
    ...
    /// Removes a sealed segment (its ZMS and sibling IDX) and prunes the day.
    ///
...
impl ZmsTemp<'_> {
    ...
    pub fn publish(&mut self) -> Result<(), LayoutError> {
        zms_publish_failpoint!(FileSync);
        self.file.sync_all()?;
        ...
        zms_publish_failpoint!(LinkedDirectorySync);
        self.day.sync_all()?;
        zms_publish_failpoint!(TemporaryUnlink);
        rustix::fs::unlinkat(&self.day, &self.temp_name, AtFlags::empty())
            .map_err(errno_to_io)
            .map_err(LayoutError::Io)?;
        zms_publish_failpoint!(UnlinkedDirectorySync);
        self.day.sync_all()?;
        self.published = true;
        Ok(())
    }
...
/// Lifetime token for the only process allowed to mutate index artifacts.
#[derive(Debug)]
...
struct ScanState<'scan> {
    ...
    orphan_indexs: Vec<SegmentAddress>,
...
fn is_writer_bootstrap_control(foreign: &ForeignEntry) -> bool {
    foreign.diagnostic.path.scope == EntryScope::Root
        && matches!(
            ...
            b"active.wal" | b".kronika-writer.owner.lock"
        )
}
...
fn rename_generation_to_active(root: &File, generation: &CStr) -> io::Result<()> {
    rename_noreplace(root, generation, root, c"active.wal")
}
...
mod tests {
    ...
    #[test]
    fn remove_sealed_segment_unlinks_the_zms_and_reports_freed_bytes() {
        let directory = tempfile::tempdir().unwrap();
        let root = DataRoot::open(directory.path()).unwrap();
    ...
    #[test]
    fn injected_zms_publication_faults_leave_only_reopenable_states() {
        let cases = [
            (
                ZmsPublishFaultPoint::FileSync,
                rustix::io::Errno::NOSPC,
                false,
                true,
            ),
            (
                ZmsPublishFaultPoint::Link,
                rustix::io::Errno::DQUOT,
                false,
                true,
            ),
            (
                ZmsPublishFaultPoint::LinkedDirectorySync,
                rustix::io::Errno::ROFS,
                true,
                true,
            ),
            (
                ZmsPublishFaultPoint::TemporaryUnlink,
                rustix::io::Errno::IO,
                true,
                true,
            ),
            (
                ZmsPublishFaultPoint::UnlinkedDirectorySync,
        ...
        for (point, errno, final_exists, temporary_exists) in cases {
            ...
            let address = address(1_709_164_801_000_000);
            let mut temporary = owner.create_zms_temp(address).unwrap();
            temporary.file_mut().write_all(b"complete ZMS").unwrap();
            let temporary_name = temporary.temp_name.clone();
            let final_path = root.diagnostic_file_path(address, FileKind::Zms);
            let raw_os_error = errno.raw_os_error();
            let fault = arm_zms_publish_fault(point, raw_os_error);
            ...
            // A killed process does not run ZmsTemp::drop. Suppress only its
    ...
    fn quarantine_rename_failure_retains_the_exact_source() {
        ...
        let address = address(1_709_164_801_000_000);
        let mut temporary = owner.create_zms_temp(address).unwrap();
        temporary.file_mut().write_all(b"damaged ZMS").unwrap();
        ...
        assert_eq!(
            std::fs::read(root.diagnostic_file_path(address, FileKind::Zms)).unwrap(),
            b"damaged ZMS"
        );
    }
    ...
    fn forensic_quarantine_scan_lists_and_reopens_canonical_evidence() {
        ...
        assert_eq!(evidence.reason(), QuarantineReason::InvalidZms);
