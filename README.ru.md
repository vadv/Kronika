...
Откройте [отчёт с синтетическими данными](https://vadv.github.io/Kronika/).
...
## Самодостаточный HTML-экспорт

В веб-интерфейсе `kronika-web` действие «Экспорт» создаёт один
самодостаточный `.html`-отчёт для выбранного интервала. `kronika-report` создаёт
такой же отчёт из одного автономного `.zms`-файла.

Один файл содержит основной интерфейс Kronika на React, выбранный ZMS и его
канонический IDX. В него также встроен движок запросов `kronika-query`: он
написан на Rust, скомпилирован в WebAssembly и выполняется в Web Worker в одном
потоке. Отчёт открывается в обычном браузере; его можно передать одним файлом
или разместить на статическом хостинге. Запущенный сервер Kronika не нужен.
После загрузки отчёт не выполняет сетевых запросов и не читает внешние ресурсы
или сопутствующие файлы. В автономном интерфейсе нет входа и аутентификации,
MCP, живого обновления и кнопки повторного экспорта.

После сборки бинарных файлов создайте отчёт из автономного ZMS:
...
`kronika-report` проверяет ZMS, строит канонический IDX и атомарно заменяет
выходной файл. Корень хранилища и предыдущий сегмент ему не нужны. Первое
значение скорости, для которого нужен более ранний отсчёт, равно `null`.
