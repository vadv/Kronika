...
строк PostgreSQL глубиной четыре с тайм-аутом последнего участника и замедленный
Vacuum отдельной таблицы.

Первый эпизод каждого типа начинается сразу. Раунд блокировок длится около
12 с. Первые участники завершают транзакции, а последний достигает явно
заданного `statement_timeout` через 10 с. PostgreSQL записывает реальные события
ожидания блокировки и отмены запроса. Затем 45 с нет демонстрационных ожиданий,
поэтому Locks и Activity показывают и инцидент, и восстановление. Генератор
повторяет эту последовательность всё время работы. Раунды ошибок, медленных
запросов и Vacuum следуют тому же сценарию «инцидент — восстановление» со
своими интервалами. Образ снимает состояние PostgreSQL раз в 5 с, поэтому
каждый ограниченный эпизод пересекается хотя бы с одним опросом коллектора.
Ни один сценарий не отключает
