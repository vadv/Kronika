...
const tree = await importModule('export { annotateProcessMetrics, buildProcessForest } from "../src/process-tree.ts"')
...
test("a simple parent/child chain walks the parent first, each child directly under it", () => {
  ...
  const forest = tree.buildProcessForest(rows)
...
test("%CPU is scheduled time over wall time since starttime, %MEM is resident over host total, TIME is scheduled seconds", () => {
  ...
  const [annotated] = tree.annotateProcessMetrics(rows, CURSOR, 100, 2_048_000)
...
test("a missing input yields a null metric instead of a wrong number", () => {
  ...
  const [withoutUtime] = tree.annotateProcessMetrics(rows, HOUR + 1_000_000, 100, 2_048_000)
  assert.equal(withoutUtime.values.cpu_time_seconds, null)
  assert.equal(withoutUtime.values.cpu_percent, null)

  const [withoutTicks] = tree.annotateProcessMetrics(rows, HOUR + 1_000_000, null, 2_048_000)
  assert.equal(withoutTicks.values.cpu_percent, null)

  const [withoutMemTotal] = tree.annotateProcessMetrics(rows, HOUR + 1_000_000, 100, null)
