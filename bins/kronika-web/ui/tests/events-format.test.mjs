...
test("event metrics use semantic precision for percent, rate, duration, zero and null", () => {
  ...
  assert.equal(events.formatMetric(6_290, "milliseconds", "en", t), "6.2 s")
  assert.equal(events.formatMetric(6_290, "milliseconds", "ru", t), "6,2 с")
  assert.equal(events.formatMetric(90_500, "milliseconds", "en", t), "1m 30s")
  assert.equal(events.formatMetric(90_500, "milliseconds", "ru", t), "1м 30с")
...
test("event identity fields remain exact while ordinary readings stay bounded", () => {
  ...
  assert.equal(events.eventValue(finding, "max_duration_ms", 6_290, "en", t), "6.2 s")
  assert.equal(events.eventValue(finding, "max_duration_ms", 6_290, "ru", t), "6,2 с")
  assert.equal(events.eventValue(finding, "elapsed_ms", 90_500, "en", t), "1m 30s")
  assert.equal(events.eventValue(finding, "elapsed_ms", 90_500, "ru", t), "1м 30с")
