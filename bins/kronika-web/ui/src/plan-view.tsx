      ...
      ? <div className="max-h-[min(320px,35vh)] overflow-auto [scrollbar-width:thin]" data-testid="pg-plan-query-list">
        <article data-testid="pg-plan-query-text">
          <div className="flex justify-end px-2 py-1.5">
            <button aria-label={t("pg.query.plan.copy_aria")} className="inline-flex min-h-7 flex-none cursor-pointer items-center gap-1 border border-line4 bg-s2 px-1.5 py-1 text-xs uppercase text-accent3" onClick={() => void navigator.clipboard?.writeText(text)} type="button"><Copy aria-hidden="true" size={12} />{t("pg.plan.copy")}</button>
          </div>
