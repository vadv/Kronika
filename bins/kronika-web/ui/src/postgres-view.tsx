...
import { databaseSummary, PostgresSummary } from "./postgres-summary"
...
export function PostgresView({
  ...
  const pageSummary = (logicalName: string) => data.snapshotRows.find((row) => row.logicalName === logicalName)?.summary
  return <>
    ...
    {section === "databases" && <><DatabasesActivity blockSize={blockSize} cursor={cursor} hour={hour} locale={locale} onCursor={onCursor} onPattern={onPattern} t={t} /><div className="lensbar flex-wrap"><PostgresSummary locale={locale} section="pg_stat_database" summary={databaseSummary(snapshot(data.sections.pg_stat_database ?? [], cursor), pattern)} t={t} /></div><PgEntityView columns={postgresByteColumns(DATABASE_COLUMNS, blockSize)} context={context} tablesLoading={tablesLoading} onContextClear={onContextClear} onCursor={onCursor} onOrder={onOrder} onPattern={onPattern} onSelectedKey={onSelectedKey} order={order} pattern={pattern} cursor={cursor} data={data} finding={focusFinding?.logicalName === "pg_stat_database" ? focusFinding : null} focus={focus} historyField="xact_commit" historyRevision={historyRevision} locale={locale} section="pg_stat_database" selectedKey={selectedKey} t={t} /></>}
