...
function clock(date: Date, locale: Locale, timeZone: string | undefined, precise: boolean): string {
  ...
  return `${parts.filter(({ type }) => type !== "timeZoneName").map(({ value }) => value).join("").trim()} ${parts.find(({ type }) => type === "timeZoneName")?.value ?? ""}`
}

function clockRange(start: Date, end: Date, locale: Locale, timeZone: string | undefined): string {
  const left = clockParts(start, locale, timeZone)
  const right = clockParts(end, locale, timeZone)
  return left.zone === right.zone
    ? `${left.time}–${right.time} ${left.zone}`
    : `${left.time} ${left.zone}–${right.time} ${right.zone}`
}

function clockParts(date: Date, locale: Locale, timeZone: string | undefined): { readonly time: string; readonly zone: string } {
  const label = clock(date, locale, timeZone, false)
  const split = label.lastIndexOf(" ")
  return { time: label.slice(0, split), zone: label.slice(split + 1) }
}

function dateRange(start: Date, end: Date, locale: Locale, timeZone: string | undefined): string {
  const format = new Intl.DateTimeFormat(locale, { day: "2-digit", month: "short", year: "numeric", ...(timeZone === undefined ? {} : { timeZone }) })
