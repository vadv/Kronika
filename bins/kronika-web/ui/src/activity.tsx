...
// One small request per (hour, cut): the server ranks next to the segments
// and answers with kilobytes, so switching cuts stays instant even over a
// slow link.
function useHeatmapView(hour: number, cut: StatementCut, revision: number): HeatmapState {
  ...
  useEffect(() => {
    ...
    loadHeatmap(hour, "pg_stat_statements", cut.field, ACTIVITY_LABELS, COLUMNS, TOP_OVERLAY, controller.signal)
...
export function StatementsActivity({ blockSize, cursor, data, hour, locale, onCursor, onRelated, segments, t }: {
  ...
  const state = useHeatmapView(hour, cut, revision)
  const view = useMemo(() => {
...
function ActivityPanel({ blockScale, cursor, cut, drill, hour, label, locale, maximized, onCursor, onCut, onMaximized, onScale, scale, t, view }: {
  ...
  return <section aria-label={t("activity.title")} className={`activity-block${maximized ? " activity-max" : ""}`} data-testid="statements-activity">
    <header className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-line px-3 py-[6px]">
      ...
      <span className="font-sans text-xs text-fg4">{t("activity.top", { top: String(view.rows.length), total: String(view.entityCount) })}</span>
...
function ActivityRow({ cells, cursor, hour, max, muted = false, onClick, onCursor, prefix = null, reading, testId, text, total }: {
