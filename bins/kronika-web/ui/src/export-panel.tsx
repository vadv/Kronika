...
export function ExportPanel({ anchor, hour, locale, mode, onActiveChange, onClose, t }: {
  ...
  useLayoutEffect(() => {
    const place = () => {
      ...
      setPosition(exportPanelPlacement(anchor.getBoundingClientRect(), {
        compact: window.matchMedia("(max-width: 520px)").matches,
  ...
  const chooseCalendarDay = (date: string) => {
    ...
    setMonth(date.slice(0, 7))
    setCalendarFocus(date)
    if (activeEndpoint === "from") setActiveEndpoint("to")
  }
  const reset = () => {
    if (activeJob.current) return
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (nextIssues.length !== 0 || fromSecond === null || toSecond === null) {
      ...
      setRequestError(null)
      focusControl(nextIssues[0]?.control ?? "from-date", {
    ...
    <form aria-busy={busy} className="flex min-h-0 flex-col" data-testid="export-form" noValidate onSubmit={(event) => { void submit(event) }}>
      <div className="export-panel-body grid min-h-0 grid-cols-[minmax(260px,1fr)_252px] gap-2.5 p-2.5 max-[640px]:grid-cols-1">
        <fieldset className="m-0 min-w-0 border-0 p-0" disabled={busy}>
          ...
          <EndpointEditor active={activeEndpoint === "from"} calendarId={calendarId} describedBy={zoneId} endpoint="from" issues={issues} onChoose={() => chooseEndpoint("from")} onDate={(value) => edit("from", "date", value)} onOccurrence={(choice) => chooseOccurrence("from", choice)} onTime={(value) => edit("from", "time", value)} refs={{ date: fromDate, occurrence: fromOccurrence, time: fromTime }} state={from} t={t} />
          <EndpointEditor active={activeEndpoint === "to"} calendarId={calendarId} describedBy={zoneId} endpoint="to" issues={issues} onChoose={() => chooseEndpoint("to")} onDate={(value) => edit("to", "date", value)} onOccurrence={(choice) => chooseOccurrence("to", choice)} onTime={(value) => edit("to", "time", value)} refs={{ date: toDate, occurrence: toOccurrence, time: toTime }} state={to} t={t} />
          <div className="mt-2 grid min-h-9 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-y border-line2 bg-s2 px-2">
            ...
            <button className="inline-flex h-7 cursor-pointer items-center gap-1 whitespace-nowrap rounded-[var(--radius-xs)] border-0 bg-transparent px-1.5 font-sans text-sm font-medium text-fg3 hover:bg-s3 hover:text-fg coarse:h-11" data-testid="export-selected-hour" onClick={reset} type="button"><RotateCcw aria-hidden="true" size={12} />{t("export.selected_hour")}</button>
          </div>
        </fieldset>
        <section aria-label={t("export.calendar")} className="min-w-0 border-l border-line2 pl-2.5 max-[640px]:border-l-0 max-[640px]:border-t max-[640px]:pl-0 max-[640px]:pt-2.5" data-testid="export-calendar" id={calendarId}>
      ...
      <div className="export-status mx-2.5 flex h-11 flex-none items-center overflow-auto border-l-2 px-2 font-sans text-sm leading-[1.35]" data-error={messages.length > 0 || job.phase === "unknown" || undefined} data-testid="export-status" id={statusId}>
      ...
      <footer className="mt-1 flex min-h-10 flex-none items-center justify-between gap-2 border-t border-line2 px-2.5 py-1">
...
function EndpointEditor({ active, calendarId, describedBy, endpoint, issues, onChoose, onDate, onOccurrence, onTime, refs, state, t }: {
