...
сессий, JSON-ответы, только инструменты. При стандартном режиме аутентификации
клиент передаёт те же учётные данные HTTP Basic, что и веб-интерфейс, в
заголовке `Authorization` каждого запроса. Без TLS учётные данные HTTP Basic
передаются в открытом виде; используйте обычный HTTP только на loopback или в
сети, которой вы явно доверяете. Эндпоинт отвергает запросы с заголовком
`Origin` и не маршрутизирует `/mcp` со строкой параметров.
`KRONIKA_WEB_AUTH=disabled` отключает проверку учётных данных, но переменные
`KRONIKA_WEB_USER` и `KRONIKA_WEB_PASSWORD` остаются обязательными.

Подстановки в примерах:

- `<URL>` — эндпоинт, например `http://127.0.0.1:8080/mcp`
